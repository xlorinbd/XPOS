...
class ReturnController extends Controller
{
            ...
            foreach ($returnss as $key=>$returns)
            {
                ...
                if(in_array("returns-edit", $request['all_permission'])) {
                    $nestedData['options'] .= '<li>
                        <a href="'.route('return-sale.edit', $returns->id).'" class="btn btn-link"><i class="dripicons-document-edit"></i> '.__('db.edit').'</a>
                        </li>';
                }
    ...
    public function create(Request $request)
    {
        ...
        if($role->hasPermissionTo('returns-add')) {
            $lims_sale_data = Sale::where([
                ...
                ['sale_status', 1]
            ])->whereNull('deleted_at')->select('id', 'sale_status','order_discount')->first();
            if(!$lims_sale_data){
            ...
            foreach ($lims_product_sale_data as $ps) {
                $ps->sold_serials = \App\Models\ProductSerial::where('sale_id', $lims_sale_data->id)
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data['total_discount'] = $request->total_sale_discount ?? ($request->total_discount ?? 0);
            ...
            $data['currency_id'] = $lims_sale_data->currency_id ?? 1;
            $data['exchange_rate'] = $lims_sale_data->exchange_rate ?? 1;
            $data['item'] = $data['item'] ?? count($data['product_id'] ?? [1]);
            ...
            foreach ($product_sale_ids as $product_sale_id) {
                ...
                if($sale_unit[$key] != 'n/a') {
                    ...
                    $hasSerials = \App\Models\ProductSerial::where('product_id', $pro_id)->exists();
                ...
                // Process returned serial numbers with strict binary ownership & idempotency
                if(!empty($imei_number[$key]) && !str_contains($imei_number[$key], "null")) {
                    $returnedImeis = array_filter(array_map('trim', explode(',', $imei_number[$key])));
                    $returnAction = $data['return_action'][$key] ?? 'restock'; // 'restock' or 'damaged'

                    foreach ($returnedImeis as $sn) {
                        $lockedSerial = \App\Models\ProductSerial::where('serial_number', $sn)
                            ->lockForUpdate()
                            ->first();

                        if (!$lockedSerial) {
                            throw new \Exception("Serial number '{$sn}' does not exist in inventory records.");
                        }

                        // Strict binary ownership & idempotency check:
                        if ($lockedSerial->status !== 'sold') {
                            throw new \Exception("Serial '{$sn}' is currently '{$lockedSerial->status}' and cannot be returned (must be in 'sold' status).");
                        }

                        if ($lockedSerial->sale_id != $lims_sale_data->id) {
                            throw new \Exception("Serial '{$sn}' does not belong to Sale #{$lims_sale_data->reference_no}.");
                        }

                        if ($returnAction === 'damaged') {
                            // Defective / RMA: marks as damaged, keeps out of salable inventory
                            $lockedSerial->markAsDamaged($data['warehouse_id']);
                        } else {
                            // Restock: marks as available, clears sale_id, syncSerialStock increments salable stock
                            $lockedSerial->markAsAvailable($data['warehouse_id']);

                            if ($lims_product_warehouse_data) {
                                $whImeis = array_filter(array_map('trim', explode(',', $lims_product_warehouse_data->imei_number)));
                                if (!in_array($sn, $whImeis)) {
                                    $whImeis[] = $sn;
                                    $lims_product_warehouse_data->imei_number = implode(',', $whImeis);
                                    $lims_product_warehouse_data->save();
                                }
                            }
                        }
                ...
                if(isset($lims_payment_data->used_points) && $lims_payment_data->used_points != null){
                    return $lims_payment_data->used_points;
            ...
            DB::rollBack();
            \Log::error("ReturnController store error: " . $e->getMessage() . " on line " . $e->getLine() . " in " . $e->getFile());
