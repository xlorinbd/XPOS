                        ...
                        <ul class="right-sidebar">
                            ...
                            <li>
                                <form id="logout-form" action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <button class="btn btn-link" type="submit"><i class="dripicons-exit"></i> {{__('db.logout')}}</button>
                                </form>
                            </li>
                        </ul>
                    </li>
            ...
            <ul id="side-main-menu" class="side-menu list-unstyled">
                @if(auth()->user()->role_users_id ==1)
                    <li class="{{ (request()->is('admin/dashboard*')) ? 'active' : '' }}"><a
                                href="{{url('/admin/dashboard')}}"> <i
                                    class="dripicons-meter"></i><span>{{__('db.Dashboard')}}</span></a>
                    </li>
                @else
                ...
                @can('view-details-employee')
                    <li class="has-dropdown {{ (request()->is('staff*')) ? 'active' : '' }}">
                        <a href="#employees" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-user-group"></i><span>{{__('db.Employees')}}</span></a>
                ...
                @can('core_hr')
                    <li class="has-dropdown {{ (request()->is('core_hr*')) ? 'active' : '' }}">
                        ...
                        <ul id="Core_hr" class="collapse list-unstyled">
                            ...
                            @can('view-promotion')
                                <li id="promotion"><a
                                            href="{{route('promotions.index')}}">{{__('db.Promotion')}}</a>
                                </li>
                            @endcan
                            @can('view-award')
                                <li id="award"><a href="{{route('awards.index')}}">{{__('db.Award')}}</a></li>
                            @endcan
                            @can('view-travel')
                                <li id="travel"><a href="{{route('travels.index')}}">{{__('db.Travel')}}</a></li>
                            @endcan
                            @can('view-transfer')
                                <li id="transfer"><a href="{{route('transfers.index')}}">{{__('db.Transfer')}}</a>
                                </li>
                            @endcan
                            @can('view-resignation')
                                <li id="resignation"><a
                                            href="{{route('resignations.index')}}">{{__('db.Resignations')}}</a>
                                </li>
                            @endcan
                            @can('view-complaint')
                                <li id="complaint"><a
                                            href="{{route('complaints.index')}}">{{__('db.Complaints')}}</a>
                                </li>
                            @endcan
                            @can('view-warning')
                                <li id="warning"><a href="{{route('warnings.index')}}">{{__('db.Warnings')}}</a>
                                </li>
                            @endcan
                            @can('view-termination')
                                <li id="termination"><a
                                            href="{{route('terminations.index')}}">{{__('db.Terminations')}}</a>
                                </li>
                            @endcan
                ...
                <li class="has-dropdown {{ (request()->is('organization*')) ? 'active' : '' }}"><a href="#Organization"
                        ...
                        <i
                                class="dripicons-view-thumb"></i><span>{{__('db.Organization')}}</span></a>
                    <ul id="Organization" class="collapse list-unstyled ">
                        @can('view-company')
                            <li id="company"><a href="{{route('companies.index')}}">{{__('db.Company')}}</a></li>
                        @endcan
                        @can('view-department')
                            <li id="department"><a
                                        href="{{route('departments.index')}}">{{__('db.Department')}}</a>
                            </li>
                        @endcan

                        @can('view-location')
                            <li id="location"><a href="{{route('locations.index')}}">{{__('db.Location')}}</a></li>
                        @endcan
                        ...
                        <li id="announcements"><a
                                    href="{{route('announcements.index')}}">{{__('db.Announcements')}}</a></li>
                ...
                @can('timesheet')
                    <li class="has-dropdown {{ (request()->is('timesheet*')) ? 'active' : '' }}"><a href="#Timesheets"
                                                                                                    aria-expanded="false"
                                                                                                    data-toggle="collapse">
                            <i class="dripicons-clock"></i><span>{{__('db.Timesheets')}}</span></a>
                        <ul id="Timesheets" class="collapse list-unstyled ">
                        {{-- @can('view-attendance') --}}
                                {{-- <li id="attendance"><a
                                            href="{{route('attendances.index')}}">{{__('db.Attendances')}}</a>
                                </li>
                    ...
                    <li class="has-dropdown {{ (request()->is('payroll*')) ? 'active' : '' }}">
                        ...
                        @if(auth()->user()->can('view-payslip') || auth()->user()->can('view-paylist'))
                            <a href="#Payroll" aria-expanded="false" data-toggle="collapse">
                                <i class="dripicons-wallet"></i><span>{{__('db.Payroll')}}</span>
                            </a>
                        @endif
                    ...
                    <li class="has-dropdown {{ (request()->is('recruitment*')) ? 'active' : '' }}">
                        ...
                        @if(auth()->user()->can('view-job_post') || auth()->user()->can('view-job_candidate')|| auth()->user()->can('view-job_interview') || auth()->user()->can('view-cms'))
                            <a href="#Recruitment" aria-expanded="false" data-toggle="collapse">
                                <i class="dripicons-user-id"></i><span>{{__('db.Recruitment')}}</span>
                            </a>
                        @endif
                    ...
                    <li class="has-dropdown @if(request()->is('training*')){{ (request()->is('training*')) ? 'active' : '' }}@elseif(request()->is('dynamic_variable/training_type*')){{ (request()->is('dynamic_variable/training_type*')) ? 'active' : '' }}@endif">
                        @if(auth()->user()->can('view-training') || auth()->user()->can('access-variable_type')|| auth()->user()->can('access-trainer'))
                            <a href="#Training" aria-expanded="false" data-toggle="collapse"> <i
                                        class="dripicons-trophy"></i><span>{{__('db.Training')}}</span>
                            </a>
                        @endcan
                        <ul id="Training" class="collapse list-unstyled ">
                            ...
                            @can('view-trainer')
                                <li id="trainers"><a
                                            href="{{route('trainers.index')}}">{{__('db.Trainers')}}</a>
                                </li>
                            @endcan
                        </ul>
                    </li>
                    ...
                    <li class="has-dropdown @if(request()->is('events*')){{ (request()->is('events*')) ? 'active' : '' }}@elseif(request()->is('meetings*')){{ (request()->is('meetings*')) ? 'active' : '' }}@endif">
                        ...
                        @if(auth()->user()->can('view-event') || auth()->user()->can('view-meeting'))
                            <a href="#Events_Meetings" aria-expanded="false" data-toggle="collapse"> <i
                                        class="dripicons-to-do"></i><span>{{__('db.Events')}} & {{__('db.Meetings')}}</span>
                            </a>
                        @endcan
                        <ul id="Events_Meetings" class="collapse list-unstyled ">
                            @can('view-event')
                                <li id="events"><a
                                            href="{{route('events.index')}}">{{__('db.Events')}}</a>
                                </li>
                            @endcan
                            @can('view-meeting')
                                <li id="meetings"><a
                                            href="{{route('meetings.index')}}">{{__('db.Meetings')}}</a>
                                </li>
                            @endcan
                        </ul>
                        @endcan
                        ...
                        <li class="has-dropdown {{ (request()->is('accounting*')) ? 'active' : '' }}">
                            ...
                            @if(auth()->user()->can('view-account') || auth()->user()->can('view-payee') || auth()->user()->can('view-payer') ||auth()->user()->can('view-deposit')||auth()->user()->can('view-expense')||auth()->user()->can('view-transaction')||auth()->user()->can('view-balance_transfer'))
                                <a href="#Finance" aria-expanded="false" data-toggle="collapse">
                                    <i class="dripicons-graph-pie"></i><span>{{__('db.Finance')}}</span>
                                </a>
                            @endcan
...
<div class="page">
    ...
    <footer class="main-footer">
        <div class="container-fluid">
            <p>&copy; {{$general_settings->site_title ?? "KGERP"}} | {{ __('Developed by')}} <span class="external">{{$general_settings->developed_by ?? 'Xlorin'}}</span> > hosting support by <a href="https://xlorinhost.com" target="_blank" class="external">Xlorinhost.com (Xlorin Host )</a></p>
        </div>
    </footer>
</div>
